...
BEGIN
  ...
  SELECT
    ...
    GREATEST(
      ...
      similarity(lower(unaccent(COALESCE(p.code, ''))), normalized_term),
      similarity(lower(unaccent(COALESCE(b.name, ''))), normalized_term),
      CASE
        WHEN lower(unaccent(p.name)) ILIKE '%' || normalized_term || '%' THEN 0.9
        WHEN lower(unaccent(COALESCE(p.code, ''))) ILIKE '%' || normalized_term || '%' THEN 0.85
